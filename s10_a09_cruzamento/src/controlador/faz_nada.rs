...
impl Controlador for FazNada {			// !!! Implementação do trait
	...
	fn estrategia(&mut self, tempo_decorrido: f64, situacao: &mut HashMap<String,Situacao>) {
		...
		for (key, val) in situacao.iter() {
			ordem_duas_vias.push( MiniSituacao{placa:val.placa.clone(), pos_atual:val.pos_atual} );
			println!("key: {}    val: {:?}", key, val);
		}
		ordem_duas_vias.sort_unstable_by(|a, b| 
							b.pos_atual.partial_cmp(&a.pos_atual).unwrap() );
		for mini in ordem_duas_vias {		// Ações para veículos nas duas vias
