...
pub enum MensagemDeVeiculo {
	Chegada{placa:String,via:Via,acel_max:f64,acel_min:f64,vel_max:f64,comprimento:f64},	// Informa que chegou
	SituacaoAtual{placa:String,pos_atual:f64,vel_atual:f64,acel_atual:f64},		// Informa a sua situação
}
...
impl Comunicacao {
	...
	pub fn send_por_controlador(&mut self, placa:String, msg:MensagemDoControlador) {
		let lista = self.mensagens_do_controlador.entry(placa).
