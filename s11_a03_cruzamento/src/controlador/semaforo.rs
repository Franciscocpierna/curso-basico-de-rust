...
fn nova_aceleracao( vel_atual:f64, vel_cruzeiro:f64, acel_min:f64, acel_max:f64) -> f64 {
	...
	println!("******* {} {}   {}   {}", vel_atual, vel_cruzeiro, acel_min, acel_max);
		...
		if self.amarelo {
			...
			for mini in ordem_via_verde {
				...
				if veiculo.vel_atual < veiculo.vel_max {
					...
					posvmax = veiculo.pos_atual + veiculo.vel_atual*tvmax + veiculo.acel_max*tvmax*tvmax/2.0; 
				}
				...
				if self.display_tudo {
					println!("#SEM @{}, {:?} em amarelo, t/passar {:.2}", veiculo.placa,  self.via_verde, tpassar);
				}
