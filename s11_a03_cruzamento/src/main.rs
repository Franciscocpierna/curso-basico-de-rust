...
use transito::{VIAH_TOTAL,VIAV_TOTAL,VIAH_MARGEM,VIAV_MARGEM};
...
impl WindowHandler for MyWindowHandler {
	fn on_draw(&mut self, helper: &mut WindowHelper, graphics: &mut Graphics2D) {
		...
		graphics.draw_line( (0.0,y1), (self.largura_total as f32,y1),
																 4.0, Color::BLACK);
		
		graphics.draw_line( (0.0,y2), (self.largura_total as f32,y2),
																 4.0, Color::BLACK);
		...
		graphics.draw_line( (zero_h,0.0), (zero_h,self.altura_total as f32),
																 4.0, Color::BLACK);
		graphics.draw_line( (x,0.0), (x,self.altura_total as f32), 
																	4.0, Color::BLACK);
		...
		for carro in self.simulacao.transito.get_iterador(Via::ViaH) {
			...
			let vertices = [ Vector2{x,y}, Vector2{x:x+w,y}, Vector2{x:x+w,y:y+h}, Vector2{x,y:y+h}];
			...
			if self.finalizada  ||  !self.simular {
				...
				if lado == 0.0 {		// Para espalhar texto na tela
					lado = -80.0;
				} else {
					lado = 0.0;
				}	
...
fn laco_simulacao( simul: &mut Simulacao) -> bool {
	...
	match simul.transito.ocorreu_colisao() {
		Some(m) => {
			println!("Ocorreu colisao, controlador {}, tempos entre {} e {}: {}", simul.cont, simul.tec_min, simul.tec_max, m);
			return false;
		}	
		None => {},
	}
	...
	if simul.tempo_ateh_proxima_chegada <= 0.0 {
		...
		match simul.transito.chega_carro( Via::ViaH, &mut simul.comunicacao) {
			Ok(_) => (),
			Err(msg) => println!("Falha em chegar um carro via H: {}", msg),
		}

		match simul.transito.chega_carro( Via::ViaV, &mut simul.comunicacao) {
			Ok(_) => (),
	...
	if simul.tempo_ateh_proximo_controle <= 0.0 {
		simul.controle.acao_controle( TEMPO_ENTRE_CONTROLES, &mut simul.comunicacao);
...
fn simula_mundo( cont: char, tec_min:f64, tec_max:f64, tam_janela:f64) {
	...
	let window = Window::new_centered("Cruzamento", (tam_janela as u32, tam_janela as u32)).unwrap();
	...
	window.run_loop(MyWindowHandler { simular: true,
												finalizada: false,
												simulacao: simul,
...
fn main() {
	...
	let cont = args[1].trim().chars().next()
	...
	let tec_min = result_tec_min.expect("Uso: <s|n>  <min entre chegadas>  <max entre chegadas> <tam janela>");

	let result_tec_max = args[3].trim().parse::<f64>();
	let tec_max = result_tec_max.expect("Uso: <s|n>  <min entre chegadas>  <max entre chegadas> <tam janela>");
	...
	if tec_min > tec_max {
		println!("Tempo entre chegadas mínimo não pode ser maior que o tempo entre chegadas máximo.");
		return;
	}


	let result_tam_janela = args[4].trim().parse::<f64>();
	let tam_janela = result_tam_janela.expect("Uso: <s|n>  <min entre chegadas>  <max entre chegadas> <tam janela>");
