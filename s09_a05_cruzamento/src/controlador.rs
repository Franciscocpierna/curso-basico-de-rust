...
impl Semaforo {
	...
	fn estrategia(&mut self, ms_decorrido: f64, situacao: &mut HashMap<String,Situacao>) {
		...
		if self.display_tudo {
			println!("nVerde {:?} {}    nVermelho {:?}    restam_amarelo {}",
...
/*	Ordenação:
	...
	Problema:
		...
		*/
		...
		ordem_via_h.sort_unstable_by(|a, b| 
										b.pos_atual.partial_cmp(&a.pos_atual).unwrap() );
		ordem_via_v.sort_unstable_by(|a, b| 
										b.pos_atual.partial_cmp(&a.pos_atual).unwrap() );
		...
		for mini in ordem_via_vermelho {
			...
			if veiculo.pos_atual >= pos_alvo {
				...
				veiculo.acel_desejada = veiculo.vel_atual.powi(2) / ( 2.0 * ( pos_alvo - veiculo.pos_atual ) );
		...
		if self.amarelo {
			...
			for mini in ordem_via_verde {
				...
				if t_chegar <= self.restam_amarelo {
					...
					// não passa
					veiculo.acel_desejada = veiculo.vel_atual.powi(2) / (pos_alvo - veiculo.pos_atual ) / 2.0;
			...
			for i in 0..ordem_via_verde.len() {
				...
				if i == 0 {  // Primeiro: acelerar até o máximo
					let veiculo = situacao.get_mut(&mini.placa).unwrap();	// Sei que a placa existe
					veiculo.acel_desejada = veiculo.acel_max;
					}
...
impl Controlador {
	...
	pub fn controle(&mut self, tempo_decorrido:f64, comunicacao: &mut Comunicacao) {
			...
			match comunicacao.receive_por_controlador() {
				...
				Some(msg) => {
					match msg {
						...
						MensagemDeVeiculo::Chegada{placa,via,acel_max,acel_min,vel_max,comprimento} => {
							let novo = Situacao {
								placa,
								via,
								acel_max,
						...
						MensagemDeVeiculo::SituacaoAtual{placa,pos_atual,vel_atual,acel_atual} => {
		...
		self.semaforo.estrategia(tempo_decorrido, &mut self.situacao);


		// Envia novas acelerações para os veículos
		for (k, v) in &self.situacao {
			let msg = MensagemDoControlador::SetAcel{placa:k.to_string(),acel:v.acel_desejada};
			comunicacao.send_por_controlador(k.to_string(), msg);

			if self.display_tudo {
				println!("#controlador setAceleracao de @{} em {}", k.to_string(), v.acel_desejada);
			}
		}
		...
		for (placa, _situacao) in &self.situacao {
			...
			let msg = MensagemDoControlador::PedeSituacao{placa: placa.to_string()};
