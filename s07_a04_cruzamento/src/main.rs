/*
	Projeto do Cruzamento Automático - versão s7
...
fn simula_carros() {
	...
	loop {
		...
		if tempo_ateh_proxima_chegada <= 0.0 {
			assert!( transito.chega_carro( Via::ViaH), "Falha em chegar um carro via H");
			assert!( transito.chega_carro( Via::ViaV), "Falha em chegar um carro via V");
