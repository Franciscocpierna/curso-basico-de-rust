...
impl Controle {
	...
	pub fn acao_controle(&mut self, tempo_decorrido:f64, comunicacao: &mut Comunicacao) {
					...
					match msg {
						MensagemDeVeiculo::Chegada{placa,via,acel_max,acel_min,vel_max,comprimento} => {
							let novo = Situacao {
								...
								vel_max,
								comprimento,
								pos_atual: match via {								// Na entrada está longe !!!
		...
		if self.tempo_ateh_proxima_solicitacao <= 0.0 {
			...
			for placa in self.situacao.keys() {		// só precisa das chaves
		...
		if self.tempo_ateh_proxima_estrategia <= 0.0 {
			...
			match &mut self.controlador {
				MeuControlador::SEMAFORO(ss) => ss.estrategia(TEMPO_ENTRE_CONTROLES, &mut self.situacao),
				MeuControlador::FAZNADA(nn) => nn.estrategia(TEMPO_ENTRE_CONTROLES, &mut self.situacao),
			}
			...
			for (k, v) in &self.situacao {
				...
				if self.display_tudo {
					println!("#controlador setAceleracao de @{} em {:.2}", k.to_string(), v.acel_desejada);
				}
			}
		}
	}
