...
impl Controlador for Semaforo {
	...
	fn estrategia(&mut self, tempo_decorrido: f64, situacao: &mut HashMap<String,Situacao>) {
		...
		if self.display_tudo {
			println!("#SEM restam verde {:?} {:.2}   restam amarelo {:.2} {}    vermelho {:?}",
									self.via_verde, self.restam_verde, self.restam_amarelo, self.amarelo, self.via_vermelho );
		}				
		...
		for (_k,v) in situacao_iter {
			if v.via == Via::ViaH {
				ordem_via_h.push( MiniSituacao{placa:v.placa.clone(), pos_atual:v.pos_atual} );
			} else {
				ordem_via_v.push( MiniSituacao{placa:v.placa.clone(), pos_atual:v.pos_atual} );
			}	
		}
		...
		for mini in ordem_via_vermelho {
			...
			if veiculo.pos_atual > 0.0 {
				...
				veiculo.acel_desejada = veiculo.vel_atual.powi(2) / ( 2.0 * ( veiculo.pos_atual - pos_alvo ) );  
			}

			if self.display_tudo {
				println!("#SEM @{}  atual:{:.2}  alvo:{:.2}  acel:{:.2}->{:.2}",
							 veiculo.placa, veiculo.pos_atual, pos_alvo, veiculo.acel_atual, veiculo.acel_desejada);
			}
		...
		if self.amarelo {
			...
			for mini in ordem_via_verde {
				...
				let tpassar = (0.0 + veiculo.comprimento + VIAH_LARGURA - veiculo.pos_atual) / 
												veiculo.vel_atual;
				if self.display_tudo {
					println!("#SEM @{}, {:?} em amarelo, t/passar {:.2}", veiculo.placa,  self.via_verde, 1000.0*tpassar);
				}
			...
			for i in 0..ordem_via_verde.len() {
				...
				if i == 0 {  // Primeiro: acelera livremente
					let veiculo = situacao.get_mut(&mini.placa).unwrap();	// Sei que a placa existe
					...
					if self.display_tudo {
						println!("#SEM @{}, {:?} em verde, vel {:.2}, acel {:.2}", veiculo.placa,  self.via_verde,
												veiculo.vel_atual, veiculo.acel_desejada);
					}
				} else {	// Veículos seguintes: acelerar mas sem bater no da frente !!!
					...
					if delta_t_atual < 2000.0 {
						...
						if veiculo.vel_atual > afrente_vel  {
							let delta_t_colisao = 1000.0 * ( afrente_pos - afrente_comp - veiculo.pos_atual ) /
					...
					if self.display_tudo {
						println!("#SEM @{}, {:?} em verde, vel {:.2}, delta_t {:.2}, acel {:.2}", veiculo.placa,  self.via_verde,
												veiculo.vel_atual, delta_t_atual, veiculo.acel_desejada);
					}
				}
			}
		}
